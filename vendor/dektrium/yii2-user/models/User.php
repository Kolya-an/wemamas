...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'achievementsUser'  => \Yii::t('user', 'Achievements'),
            'bioUser'           => \Yii::t('user', 'Bio'),
            'categoryUser'      => \Yii::t('user', 'Category'),
            'profileUser'       => \Yii::t('user', 'profileUser'),
            'locationUser'      => \Yii::t('user', 'Location'),
            'officeUser'        => \Yii::t('user', 'Office'),
            'websiteUser'       => \Yii::t('user', 'Website'),
            'phoneUser'         => \Yii::t('user', 'Phone'),
            'birthday'          => \Yii::t('user', 'Birthday'),
            'subscribe'         => \Yii::t('user', 'Subscribe'),
            'subscribeUser'     => \Yii::t('user', 'Subscribe'),
            'salaryUser'        => \Yii::t('user', 'Salary'),
            'workUser'          => \Yii::t('user', 'Work'),
            'experienceUser'    => \Yii::t('user', 'Experience'),
            'percentUser'       => \Yii::t('user', 'Percent'),
            'photoUser'         => \Yii::t('user', 'Photo'),
            'logoUser'          => \Yii::t('user', 'Logo'),
        ];
    }
    ...
    public function scenarios()
    {
        return [
            ...
            'connect'  => ['username', 'email', 'category_id'],
            'create'   => ['username', 'email', 'password', 'category_id', 'subscribe'],
            'update'   => ['username', 'email', 'password', 'category_id', 'subscribe', 'bio'],
            'settings' => ['username', 'email', 'password', 'category_id', 'subscribe'],
        ];
    }
    ...
    public function rules()
    {
        return [
            ...
            //category
            'categoryLength'   => ['category_id', 'integer'],
...
//                'gravatar_email' => $this->email,
   ...
   public function getCategory()
    ...
    public function getAchievementsUser() 
    {
    return $this->category->achievements;
    }
    public function getBioUser() 
    {
    return $this->category->bio;
    }
    ...
    {
    return $this->profile->salary;
    }
    public function getWorkUser() 
    {
    return $this->profile->work;
    }
    ...
    if ($this->profile->subscribe === 1) {
        $subscribe = 'Подписан';
    } else {
        $subscribe = 'Не подписан';
    }
